...
pipenv install
pipenv shell
The lib folder contains starter code for the Department and Employee classes. Both classes include ORM methods for mapping the individual class to a corresponding table in the database:

Python Class	Relational Database Table
Department	departments
Employee	employees
The one-to-many relationship between Department and Employee is not implemented in the starter code. In this lesson, we will:

Update the Employee class to add a new attribute to model the one-to-many relationship.
Update the Employee class methods to store and manage the relationship in the "employees" database.
Update the Department class to add a new method to compute (not store) a list of associated Employee instances.
...
Employee is on the many side of the relationship, thus it owns and manages the relationship by storing a single piece of data about the associated Department.

The "Employee" class will have an attribute department_id to reference the id of the associated Department object.
The "employees" table will contain a column department_id to store a foreign key reference to the "departments" table.
Department is on the "one" side of the relationship, thus it should not store a list of the associated Employee instances. Instead, a list of the many Employee objects associated with the department will be computed by looking at the department_id foreign key value stored with each employee.

Updating the Employee class to store the relationship
Let's update the Employee class to store the relationship to Department by making the following changes:

Import the Department class.
Update the __init__ method to add a parameter department_id and store the value in a new attribute with the same name.
...
Next, we'll update the create_table method to add a column named department_id to store the relationship as a foreign key in the "employees" table:
...
def create_table(cls):
    ...
    CURSOR.execute(sql)
    CONN.commit()
We need up adapt the save, update, and create methods to persist the new relationship by storing the department id in the "employees" table:
...
The final change to the Employee class is to update the instance_from_db method to include the department id from the table row.
...
Since Department is not on the owning side of the relationship, we won't store anything about employees in the "departments" table. But we might want to get a list of all employees that work for a department. We'll add a method to the Department class that accomplishes this task:
...
Note that we import the Employee class within the function definition to issues with circular imports (since the Employee class imports the Department class as well).
...
The file lib/debug.py has been updated to initialize the database with two departments and several employees. The code generates fake names for the employees, and picks a random job title and department for each employee.
...
The lib/testing folder contains the files department_orm_test.py and employee_orm_test.py to test the Department and Employee classes. Run the tests and confirm they pass:
